# Goodrich, Tamassia, Goldwasser (2013) Code Fragment 14.19 on page 685
...
class Partition:
    ...
    def find(self, p):
        ...
        if p._parent != p:
            p._parent = self.find(p._parent)      # overwrites p._parent after recursion (path compression)
        return p._parent
    ...
    def union(self, p, q):
        ...
        if a is not b:                                         # only merge if different groups
            if a._size > b._size:                         # (union-by-size)
                b._parent = a
                a._size += b._size
            else:
                a._parent = b
